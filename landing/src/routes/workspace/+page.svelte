<script lang="ts">
	...
	onMount(() => {
		if (window.self === window.top) rememberView('workspace', browserStorage());
...
<section class="flex h-full flex-col bg-surface-50 text-surface-900 dark:bg-surface-950 dark:text-warm-50">
	<header class="flex flex-wrap items-center justify-between gap-3 border-b border-surface-200 px-4 py-3 dark:border-surface-800 sm:px-6">
		<div class="flex items-center gap-3">
			<a href="/?view=classic" aria-label="Aylith classic home" class="inline-flex items-center gap-2 font-semibold tracking-wide"><Mark class="h-6 w-auto" /> Aylith</a>
		...
		<div class="flex items-center gap-4 text-sm">
			...
			<a href="/?view=classic" class="text-surface-600 hover:underline dark:text-warm-300">Classic site</a>
		</div>
	</header>
	<div class="flex flex-wrap items-center justify-between gap-2 border-b border-surface-200 bg-white/70 px-4 py-2 dark:border-surface-800 dark:bg-surface-900/50 sm:px-6">
		...
		<a href={active.id === 'home' ? '/?view=classic' : active.url} target="_blank" rel="noopener noreferrer" class="text-sm font-semibold text-accent-700 underline-offset-4 hover:underline dark:text-accent-400">Open in new tab ↗</a>
	</div>
