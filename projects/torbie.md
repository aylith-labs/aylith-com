...
features:
  - >-
    Loads plugins written for Tabby unchanged — the compatibility contract is
    deliberate
  - >-
    Records what blocks an event loop, in the main process and every renderer,
  ...
  - >-
    Health-checks every build on the machine and names the cause when one will
    not start
  - >-
    A watchdog, so a process that can never show a window cannot hold the
